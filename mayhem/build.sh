#!/usr/bin/env bash
#
# cryptsetup/mayhem/build.sh — build cryptsetup's three OSS-Fuzz LUKS-header harnesses as sanitized
...
#   crypt_load_misc_fuzz    — writes the raw input and tries crypt_load(VERITY), then INTEGRITY.
#                             Exercises the dm-verity + dm-integrity superblock parsers.
# All three are OSS-Fuzz harnesses (tests/fuzz/oss-fuzz-build.sh ENABLED_FUZZERS) — ship them all
# (§6.2 item 12). `make fuzz-targets` builds every EXTRA_PROGRAM; we install + relink each below.
...
for fuzzer in crypt2_load_fuzz crypt2_load_ondisk_fuzz crypt_load_misc_fuzz; do
...
ls -la "$OUT/crypt2_load_fuzz" "$OUT/crypt2_load_ondisk_fuzz" "$OUT/crypt_load_misc_fuzz" \
       "$OUT/crypt2_load_fuzz-standalone" "$OUT/crypt2_load_ondisk_fuzz-standalone" \
       "$OUT/crypt_load_misc_fuzz-standalone" \
